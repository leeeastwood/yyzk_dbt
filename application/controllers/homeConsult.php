...
class HomeConsult extends CI_Controller {
...
//     //If no session, redirect to login page
    ...
    try{
            ...
            $crud->where('UserID',$data['id']);
            $crud->columns('cStatus','cTimestamp','eName','eContractPerson','eContractCellphone');
            $crud->display_as('eName','企业')
                 ...
                 ->display_as('eApplyLoanCount','本次贷款金额（万）')
          ...
          //  $crud->add_action('详细', '', 'demo/action_more','ui-icon-plus');   
            ...
            $crud->unset_fields('cTimestamp','cStatus','cSubmitTime','cReplyTime','cAssignto','cReplyContent','ConsID');
          //  $crud->unset_add_fields('cTimestamp','cStatus','cSubmitTime','cReplyTime','cAssignto','cReplyContent');
...
{          
       ...
       $files = array(
        ...
        // var_dump($files);
        $this->load->view('home_headleft',$files);	
       $this->load->view('home_consult_view',$output);	
