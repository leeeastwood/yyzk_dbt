...
{	
    ...
    /**
	 * lidong add support view button to show the field 
	 */
		...
		switch ($real_type) {
			...
			case 'datetime':
				...
				//	$value = date ("d M Y - H:i", mktime ( (int)$hours , (int)$minutes ,0, (int)$month , (int)$day ,(int)$year));
					$value = date ("Y-m-d H:i", mktime ( (int)$hours , (int)$minutes ,0, (int)$month , (int)$day ,(int)$year));
				}
				else 
				{
					$value = '';
	...
	protected function db_insert($state_info)
	{
		...
		if($validation_result->success)
		{
			...
			if($this->callback_insert == null)
			{
                ...
                // real insert by calling the model
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
        ...
        // lidong add, to surpport the view button action column
 ...
 /**
 * lidong 
 * to support the canedit callback, dynamically disable the edit/delete button based on the content of row            
 *             
 *             
 */
            $can_edit = true;
			if (isset($this->callback_canedit)) {
				$can_edit = call_user_func($this->callback_canedit,$row->{$data->primary_key});
			} 
		       	if ($can_edit) {
				$data->list[$num_row]->edit_url = $data->edit_url.'/'.$row->{$data->primary_key};
				$data->list[$num_row]->delete_url = $data->delete_url.'/'.$row->{$data->primary_key};
 ...
 */        
	...
	protected function change_list_add_actions($list)
	{
		...
		foreach($list as $num_row => $row)
		{
			...
			foreach($this->actions as $unique_id => $action)
			{
				...
				{
					 $urlresult = call_user_func($action->url_callback, $row->$primary_key, $row); 
                    
                    if($urlresult === 'op/unset/yes'){
                        unset($this->action[$unique_id]);
                    }else {
                        $actions_urls[$unique_id]= $urlresult;
                    }
    ...
    /**
     * lidong add, to support the click of view button ,show the view form page
     */
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
    ...
    /**
     * lidong add, support view button 
     */
...
class grocery_CRUD extends grocery_CRUD_States
{
    ...
    // lidong add for canedit
    protected $callback_canedit         = null; 
    //lidong add for canedit		
...
//@todo ui_day, ui_month, ui_year has to be lang strings
	...
	public function callback_before_insert($callback = null)
	{
  	 ...
  	 /**
	 * 
	 * Lidong: add a callback function for the use of dynamically disable the action button 
	 */
    public function callback_canedit($callback = null)
	{
		$this->callback_canedit = $callback;
		return $this;
	}
